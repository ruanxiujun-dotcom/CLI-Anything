# Test Plan â€?cli-anything-joplin

The suite has two files:

- `test_core.py` â€?pure unit + CLI-contract tests (no `joplin` binary required)
- `test_full_e2e.py` â€?CLI subprocess tests + real-backend workflows + full
...
- Session lifecycle: set/get, snapshot, undo, redo, undoâ†’redo chain,
...
- `session save` keeps undo/redo depth consistent across save â†?undo â†?redo â†?save
...
- **Note organization** â€?copy, rename (via `ren`), move (via `mv`) between
  notebooks
- **Todo lifecycle** â€?create todo â†?list â†?done â†?undone â†?toggle â†?clear
- **Tagging** â€?add, list, notetags, tagnotes, remove
- **Search** â€?best-effort; tolerates the GUI-mode restriction by checking the
  error message when the backend rejects `search` outside the REPL
- **Sync (no target)** â€?verifies `sync run` returns a payload even when no
  target is configured
- **Export** â€?JEX and Markdown
- **Import** â€?Markdown directory import
- **Attach** â€?attach file to note, then read it back
- **Unicode** â€?skipped on Windows because `joplin.cmd` truncates non-ASCII
  args; the Python-only unicode roundtrip in `test_core.py` covers harness
  encoding
- **Session history persists** â€?verifies history actions land in the saved
  project file
...
`todo.toggle`, `tag.add`, `attach.add`, `interop.export`, â€?.
...
- Non-ASCII process arguments on Windows pass through `joplin.cmd` â†?
